% Load the predictions on the "unlabeled" data which have been generated using 
% NCC template matching and create an IMDB with training data from it
%
...
labelNIIs = getAllFiles(options.data.dir, '*prior-extra.nii.gz', true);
...
labelNIIs = [labelNIIs; getAllFiles(options.data.dirMSKRI, '*prior-extra.nii.gz', true)];
...
for i=1:numImages
    ...
    sz = nii_imageA.size();
    bitDepth = nii_imageA.nii.hdr.dime.bitpix;
...
% Save RUSDB
tmp = strrep(options.data.imdbDir, '.mat', '-prior-extra.mat');
save(tmp, 'imdb', '-v7.3');
toc;