...
% 3D templates
% -> doing so, determine the respective domain of the volume and add the
% templates to the templates for that respective domain
...
options.prior.templatePatients = {'1', '4', '7', '10'};
...
for i=1:numImages
    ...
    % properties
    if isempty(templates{domainIdx})
        templates{domainIdx} = zeros(options.prior.templateSize(1), options.prior.templateSize(2), options.prior.templateSize(3), 0);
    end
    for s=1:numel(S)
        component = S(s);
        ...
        %   component.FilledArea > round(0.01 * options.prior.templateSize(1) * options.prior.templateSize(2) * options.prior.templateSize(3)) 
            ...
            templates{domainIdx}(:,:,:,end+1) = template;
...
for t=1:numel(templates)
    ridx = randperm(size(templates{t},4), min(size(templates{t},4), options.prior.numTemplatesPerDomain));
    templates{t} = templates{t}(:,:,:,ridx);
end

% Save Templates
save([options.data.dir '/templates.mat'], 'templates', '-v7.3');

% Next step: compute optimal operating points on exactly the volumes of the
% templatePatients
operatingPoints = [];
for i=1:numImages
    nii_image = NII(images{i});
    ...
    current_templates = templates{domainIdx};
    ...
    operatingPoints(domainIdx) = getOptimalThreshold( prior(:), labels(:), 4 );
end
...
for i=1:numImages
    ...
    % Perhaps compute the optimal operating point?
    ...
    options.prior.threshold = operatingPoints(domainIdx);
    labels = nii_labels.getData();
    ...
    filename_prior = [pathstr '/prior-intra.nii'];
