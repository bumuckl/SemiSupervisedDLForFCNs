% Same as createIMDB, but with maxPatches constraint to reduce the number of training samples
%
...
% Options
CNN_opts;
options.data.maxPatches = 200;
options.data.positivePixelPercentage = 0.005; % 0.5 Percent of positive pixels are required per patch
        ...
        for r=1:length(options.data.rotations) % Rotate the full images to speed up patch extraction
           ...
           for s=1:length(options.data.scales)
               ...
               for n=1:length(options.data.noise_sigmas)
                   % Do not add anymore patches when we reached the limit
                   % of maxPatches. Instead, break all the loops
                   if size(imdb.images.data,4) > options.data.maxPatches
                       break;
                   end
                   ...
                   elseif options.data.randomCrops
                       ...
                       while c < options.data.randomCropsPerAugmentation
                           ...
                           % If the cropped patch has less than
                           % positivePixelPercentage positive pixels, retry
                           if sum(Image.labelmap_randomcrop(:) == 2)/numel(Image.labelmap_randomcrop) < options.data.positivePixelPercentage
                               continue;
                           end
