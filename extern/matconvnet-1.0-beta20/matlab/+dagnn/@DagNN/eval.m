...
%   tensoridx = obj.getVarIndex(obj.layers(l).outputs{1}), quickstats(obj.vars(tensoridx).value), quickview(obj.vars(tensoridx).value);
%   pause(1);
%   end
  
  %varindex = obj.getVarIndex(obj.layers(l).outputs{1});
  %disp(['Layer ' obj.layers(l).name ' statistics: min: ' num2str(min(obj.vars(varindex).value(:))) ' max: ' num2str(max(obj.vars(varindex).value(:))) 'mean: ' num2str(mean(obj.vars(varindex).value(:)))]);
