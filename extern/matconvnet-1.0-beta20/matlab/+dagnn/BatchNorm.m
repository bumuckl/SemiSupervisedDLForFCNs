classdef BatchNorm < dagnn.ElementWise
  ...
  methods
    function outputs = forward(obj, inputs, params)
      ...
      %global BN;
      %BN.(obj.net.mode).mu(end+1) = 0;
      %BN.(obj.net.mode).std(end+1) = 0;
      %disp(['BatchNorm: Moments: Min: ' num2str(min(params{3}(:))) ' Mean: ' num2str(mean(params{3}(:))) ' Max: ' num2str(max(params{3}(:)))]);
    end
