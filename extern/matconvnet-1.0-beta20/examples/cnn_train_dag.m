...
% -------------------------------------------------------------------------
sel = find(cellfun(@(x) (isa(x,'dagnn.Loss') || isa(x,'dagnn.DiceLoss') || isa(x,'dagnn.CWSMCELoss') || isa(x,'dagnn.SemiSupervisedEmbeddingLoss') || isa(x,'dagnn.SoftmaxLogLoss')) || isa(x, 'dagnn.EmbeddingLoss'), {net.layers.block})) ;
stats = struct() ;
for i = 1:numel(sel)
