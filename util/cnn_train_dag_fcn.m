function [net,stats] = cnn_train_dag_fcn(net, imdb, getBatch, varargin)
...
for epoch=start+1:opts.numEpochs
  ...
  if numel(opts.gpus) <= 1
    [currentStats, prof] = process_epoch(net, state, opts, 'train') ; 
    if isfield(stats, 'train')
        currentStats = orderfields(currentStats, stats.train);
    end
    stats.train(epoch) = currentStats;
    %[stats.train(epoch),prof] = process_epoch(net, state, opts, 'train') ;
    
    currentStats = process_epoch(net, state, opts, 'val') ; 
    if isfield(stats, 'val')
        currentStats = orderfields(currentStats, stats.train);
    end
    stats.val(epoch) = currentStats;
...
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    % New: make sure the data is a gpuArray if needed
    if numGpus >= 1
...
for p=1:numel(net.params)
  ...
  switch net.params(p).trainMethod
	...
	% Custom solution: do a real averaging rather than a running exponential average which heavily relies on the first batch for initialization
	  %net.params(p).value = (batchSize*state.epoch*net.params(p).value + net.params(p).der) ./ (batchSize*state.epoch+1);
    ...
    case 'gradientWObatchsize'
      thisDecay = opts.weightDecay * net.params(p).weightDecay ;
      thisLR = state.learningRate * net.params(p).learningRate ;
      if net.params(p).der(1) ~= 0
          state.momentum{p} = opts.momentum * state.momentum{p} ...
            - thisDecay * net.params(p).value ...
            - net.params(p).der ;
          net.params(p).value = net.params(p).value + thisLR * state.momentum{p} ;
          disp(['*update* beta: ' num2str(net.params(p).value)]);
      end
    case 'otherwise'
...
% -------------------------------------------------------------------------
sel = find(cellfun(@(x) (isa(x,'dagnn.Loss') || isa(x,'dagnn.CWSMCELoss') || isa(x,'dagnn.CWSMCELoss2') || isa(x,'dagnn.DiceLoss') || isa(x,'dagnn.FbetaLoss') || isa(x,'dagnn.FbetaLossMulticlass') || isa(x,'dagnn.MCCLoss') || isa(x,'dagnn.SemiSupervisedEmbeddingLoss') || isa(x,'dagnn.CORALLoss') || isa(x,'dagnn.SoftmaxLogLoss')) || isa(x, 'dagnn.EmbeddingLoss'), {net.layers.block})) ;
stats = struct() ;
for i = 1:numel(sel)
