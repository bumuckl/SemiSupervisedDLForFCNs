...
% Nesterov: compute the gradient on approximate prediction of the networks params
net_interim = net;
for p=1:numel(net.params)
	net_interim.params(p).value = net_interim.params(p).value - opts.momentum * state.momentum{p};
end
net_interim.eval(state.getBatch(state.imdb, state.batch), opts.derOutputs);
...
for p=1:numel(net.params)
  ...
  switch net.params(p).trainMethod
    ...
    case 'gradient'
      ...
      state.momentum{p} = opts.momentum * state.momentum{p} ...
        - thisDecay * net.params(p).value ...
        - (1 / batchSize) * net_interim.params(p).der ;
      net.params(p).value = net.params(p).value + thisLR * state.momentum{p} ;
