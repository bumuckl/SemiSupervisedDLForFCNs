...
% @Author: Christoph Baur
    ...
    for i=1:length(imagestruct)
        ...
        Eval.TP = Eval.TP + sum(Image.responseMapTP(:)); %Image.labelsWithResponses(:)
        Eval.FP = Eval.FP + sum(Image.responseMapFP(:));
        ...
        Eval.Patient(patient).TP = Eval.Patient(patient).TP + sum(Image.responseMapTP(:));
        Eval.Patient(patient).FP = Eval.Patient(patient).FP + sum(Image.responseMapFP(:));
